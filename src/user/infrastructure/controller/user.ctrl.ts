...
export class UserController {
  ...
  public async loginCtrl({ body }: Request, res: Response) {
    ...
    if (!taskResult.success) {return res.status(400).json({token:null, error: JSON.parse(taskResult.error.message) })}
    const UserExists = await this.userUseCase.getValidateEmail(email);    
    if(!UserExists) return res.status(401).json({message:"Usuario Inexistente."});
    const passwordHash = UserExists.password; //TODO el encriptado!
    const isCorrect = await verified(password, passwordHash);
    if(!isCorrect) return res.status(401).json({token:null,error:"Datos Incorrectos."});
